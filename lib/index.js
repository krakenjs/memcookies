...
module.exports = function memCookies(configuration) {
    ...
    /*
         ...
         For apps which need to support cookies-disabled mode. For this, we need to store cookies somewhere
    ...
    return function (req, res, next) {
        ...
        if (xCookies) {
            ...
            req.headers.cookie = Object.keys(xCookies).map(function (encrypted_key) {
                ...
                var key = decrypt(configuration.encryptionKey, encrypted_key);
                var payload = parseData(configuration.encryptionKey, decrypt(xCookies[encrypted_key]));
        ...
        if (xCookies || !req.xhr) {
            ...
            onHeaders(res, function () {
                ...
                rawCookies.forEach(function (cookie) {
                    ...
                    cookies[encrypt(configuration.encryptionKey, cookie.name)] = encrypt(configuration.encryptionKey, JSON.stringify(payload));
                });
...
module.exports.MemCookiesError = MemCookiesError;